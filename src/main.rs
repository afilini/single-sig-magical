...
use bitcoin::{Network, Address, util::bip32::ExtendedPrivKey};
...
async fn main() -> Result<(), WalletError> {
    ...
    let mnemonic = Mnemonic::from_phrase(mnemonic, Language::English).map_err(|e| WalletError::Mnemonic(format!("{:?}", e)))?;
    ...
    let client = Client::new("electrum.blockstream.info:60001").await?;
    let database = sled::open("magical-db")?.open_tree("default")?;

    let wallet = Wallet::new(&desc, Some(&change_desc), Network::Testnet, database, ElectrumBlockchain::from(client)).await?;
    ...
    if balance > 0 {
        ...
        let (psbt, details) = wallet.create_tx(vec![(receiver, 0)], true, 1e3 * 1e-8, None, None, None)?;
        println!("PSBT created, total outgoing amount = {}. Signing now...", details.sent);
        ...
        let txid = wallet.broadcast(raw_tx).await?;
        println!("Tx {txid} sent! Explorer: https://blockstream.info/testnet/tx/{txid}", txid = txid);
    } else {
